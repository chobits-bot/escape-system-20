...
void send_buzzer()//定时发送数据且蜂鸣器响应
{
  ...
  if(MQTT_send_count == Mesh_Count)
  {
    ...
    messageString += "}}";
    // mesh.sendBroadcast(messageString);//mesh发送 - CAT1模式下不用
    // 通过CAT1 MQTT发送数据
    Serial2.println("AT+MQTTPUB=" + topicString + ",0,0");
    delay(10);
...
void setup()
{
  ...
  digitalWrite(buzzer,LOW);
  // 初始化WiFi以获取MAC地址（CAT1模式不需要连接WiFi）
  WiFi.mode(WIFI_STA);
  WiFi.begin();
  delay(1000); // 等待WiFi初始化获取MAC地址
  Serial.print("设备MAC地址: ");
  Serial.println(WiFi.macAddress());
  Wire.begin();
  // 初始化AHT20温湿度传感器
  AHT.begin();
  Serial.println("AHT20传感器初始化完成");
  // 启动ENS160空气质量传感器
  while( NO_ERR != ENS160.begin() ){
    Serial.println("请检查ENS160传感器！");
    delay(1000);
  }
  ENS160.setPWRMode(ENS160_STANDARD_MODE);
  ...
  /**********************************************************************/
  init_EICEG();
  // CAT1模式：初始化Serial2用于与CAT1模块通信
  Serial2.begin(115200, SERIAL_8N1, 35, 32);//RX=35, TX=32
  Serial.println("CAT1模块串口初始化完成");
  // 初始化CAT1模块MQTT连接
  Instruction_Mode();
  dialog_Mode();
  MQTT_Parameter_Settings();
  Setting_MQTT_server_parameters(MQTT_SERVER);
  Setting_the_ClientID_parameter("ESP32_" + WiFi.macAddress());
  Setting_the_User(MQTT_USER);
  Setting_the_PSW(MQTT_PASSWORD);
  // 主题设置
  topicString = "ESP32/" + WiFi.macAddress() + "/status";
  theme_re = "ESP32/" + WiFi.macAddress() + "/public";
  Setting_MQTTSUB();
  Serial.println("CAT1 MQTT配置完成");
