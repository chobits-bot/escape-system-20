...
void receiveCallback(byte *payload) // 收到信息后的回调函数
{
  ...
  if((dynamic_static_mid!=mid_temporarily)&dynamic_static_mid==0)
  {
    silky_mid = 1;
  }
  setDirectionAndDynamic(namedis_right, &dynamic_static_right, &direction_LED_right, &DLED_Enable_right);
  if((dynamic_static_right!=right_temporarily)&dynamic_static_right==0)
  {
    silky_right = 1;
  }
  /******************场效应管***********************/
...
void send_buzzer()//定时发送数据且蜂鸣器响应
{
  ...
  /*************发送数据**************/
  if(MQTT_send_count == MQTT_Count)
  {
    String messageString = "";
    
    MQTT_send_count = 0;

    // 使用 ArduinoJson 构建 JSON，避免手动拼接字符串
    StaticJsonDocument<768> doc; // 根据数据量调整大小
    String deviceId = "ESP32_" + WiFi.macAddress();
    JsonObject myData = doc.createNestedObject(deviceId);
    ...
    myData["temp"] = temp;
    myData["hum"] = hum;
    myData["electricity_Current"] = electricity_Current;
    myData["electricity"] = electricity_data;
    myData["ESP32_fires_flag"] = ESP32_fires_flag;
    myData["ESP32_electricity_flag"] = ESP32_electricity_flag;
    myData["buzzer"] = buzzer_flag;
    ...
    if(mesh_number > 0)
    {
      messageString = "[" + jsonOutput;
      for(int i=0;i<mesh_number;i++)//发送来自不同设备的数据
      {
        messageString += ",";
        messageString+=mesh_dat[i];
      }
      messageString += "]";
      clean_mesh();//清除储存的数据
    } else {
...
//     String formattedMac = rawMac;  
//     formattedMac.replace(":",  "");  // 移除原始冒号（根据实际需求调整）  
//     formattedMac.toLowerCase();      // 统一为小写  
...
//     //建立发送内容
//     messageString += "{\"mac\": \"";    // 左引号  
//     messageString += formattedMac;  
//     messageString += "\"";             // 右引号+逗号 
//     messageString += ",\"AQI\": ";
...
boolean check()//监测是否断电
{
    ...
    return (sum/8) > 300 ? true : false ;//true-有电
}
...
void xTaskTwo(void *xTask2)
{
  ...
  while (1)
  {
    ...
    if(Serial2.available())
    {
      ...
      if((serialData)!="")
      {
        String Mesh_Data = serialData;
        receiveCallback((byte*)serialData.c_str());
        mesh.sendBroadcast(Mesh_Data);//mesh发送
      }
    }
    tasker1.update();
    mesh.update();
...
void setup()
{
  ...
  while( NO_ERR != ENS160.begin() ){
    Serial.println("ENS160 not found, skipping...");
    break;
  }
  // ENS160 not present, skip config
  // ENS160.setPWRMode(ENS160_STANDARD_MODE);
